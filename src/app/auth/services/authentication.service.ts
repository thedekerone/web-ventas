...
export class AuthenticationService {
  ...
  constructor() {
    ...
    this.currentUserSubject = new BehaviorSubject<User>(JSON.parse(localStorage.getItem('currentUser') || JSON.stringify(this.userLogOut)));
  ...
  login(login: string, password: string) {
    ...
    /*return this._backOfficeService.login(login, password)
      .pipe(
        map( (user: User)  => {
          if (user && user.token) {
            user.correo = login;
